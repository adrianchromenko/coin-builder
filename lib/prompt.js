...
// ---------- the described coin ----------
// The simple flow: the customer says what the coin is for and describes each face in their own words. The prompt
// is assembled from that. Exact wording is whatever they put in double quotes, which is also what gets proofread.

const PURPOSES = {
  celebration: { label: 'Celebration', hint: 'a celebratory keepsake: festive but tasteful, something people are proud to hand out' },
  branding: { label: 'Corporate branding', hint: 'a corporate branded coin: clean, professional and brand-forward, the logo is the hero' },
  anniversary: { label: 'Anniversary', hint: 'an anniversary coin: dignified and commemorative, with the years or dates given pride of place' },
  souvenir: { label: 'Event souvenir', hint: 'an event souvenir: memorable and collectible, clearly tied to the event' },
};
const normalizePurpose = (key) => (PURPOSES[key] ? key : '');
 ...
 *   purpose      key from PURPOSES (may be empty)
 ...
 */
function buildDescribedPrompt({ purpose, style, description, sideName = 'front', hasLogo = false, hasFront = false, referenceCount = 0, feedback = '' }) {
  ...
  if (purpose) sections.push(`PURPOSE\nThis is ${PURPOSES[purpose].hint}. Let that guide the mood and the ornament, without adding anything the customer did not ask for.`);
  ...
  sections.push(
    'THE PHOTO\n'
    + (hasFront
      ? 'A sharp, professional product photograph of one die-struck coin in exactly the shape of IMAGE 1: real metal with crisp relief, believable depth, fine surface texture and natural reflections. '
      : 'A sharp, professional product photograph of one perfectly round die-struck coin (unless the customer asked for another shape): real metal with crisp relief, believable depth, fine surface texture and natural reflections. ')
    + 'The coin is seen straight on (not tilted), centered, filling most of the frame, fully inside the frame, under soft even studio lighting on a plain dark charcoal background. '
...
module.exports = {
  FINISHES, COLORS, SHAPES, ADDONS, RIMS, PURPOSES,
  normalizePurpose, freeText, quotedPhrases, guessFinish, buildDescribedPrompt,

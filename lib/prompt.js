...
const normalizeShape = (key) => (SHAPES[key] ? key : DEFAULT_SHAPE);
...
const RIMS = {
  ...
  rope: 'a twisted rope border just inside the raised rim',
  beads: 'a ring of small evenly spaced raised beads just inside the raised rim',
  stars: 'a ring of small evenly spaced raised five-pointed stars just inside the raised rim',
};
...
// The coin lettering, cleaned the same way the layout preview shows it
function normalizeTexts(t) {
  const clean = (v, max) => String(v || '').replace(/\s+/g, ' ').trim().toUpperCase().slice(0, max);
...
function buildPrompt({ finish, color, shape, addons, texts, hasLogo = true, border, background, centerFirstLineWords = 0, referenceCount = 0, feedback = '' }) {
  ...
  sections.push(
    ...
    + (lines.length
      ? `The coin has exactly ${lines.length} line${lines.length === 1 ? '' : 's'} of lettering, all capitals:\n`
        ...
        + 'Render it as raised metal letters with flat polished tops in a bold, plain, evenly spaced serif capital typeface, big enough to read at a glance. '
  ...
  build.push(odd
    ? 'Shape: a custom die-cut coin, NOT a circle. Cut the outer edge of the metal to follow the silhouette of the central artwork, with an even raised border hugging that contour and a smooth polished edge. '
      + 'Use the round outline in IMAGE 1 only as a placement guide, and keep all of the lettering, fitted inside the new outline and reading exactly the same.'
    : `Shape: a perfectly round coin with a raised outer rim and ${has('edge-text') ? 'a smooth polished edge' : 'a subtle reeded edge'}.`);
  if (has('two-tone-one') || has('two-tone-both')) build.push(`2-tone plating: the raised details, lettering and rim are in the main plating, and the recessed background is plated in contrasting ${contrastMetal(finish)}.`);
...
module.exports = {
  FINISHES, COLORS, SHAPES, ADDONS, RIMS,

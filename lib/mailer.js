...
async function sendOrderEmail(order, image) {
  ...
  const rows = [
    ...
    ['Size', `${order.size}"`],
    ['Metal finish', order.finishLabel || order.finish],
    ['Color', d.colorLabel],
    ['Shape', d.shapeLabel],
    ['Center background', [bg.name || bg.color, bg.texture && bg.texture !== 'smooth' ? bg.texture : ''].filter(Boolean).join(', ')],
    ['Rim style', d.border],
    ['Top text', d.topText],
    ['Center text', d.centerText],
    ['Bottom text', d.bottomText],
    ['Logo file', d.logoName],
    ['Back', d.back ? 'Designed by the customer (right-hand coin in the artwork)' : 'Not designed in the builder'],
    ['Back top text', d.back && d.back.topText],
    ['Back center text', d.back && d.back.centerText],
    ['Back bottom text', d.back && d.back.bottomText],
    ['Back background', d.back && [d.back.background && (d.back.background.name || d.back.background.color), d.back.background && d.back.background.texture !== 'smooth' ? d.back.background.texture : ''].filter(Boolean).join(', ')],
    ['Back rim style', d.back && d.back.border],
    ['Back logo file', d.back && d.back.logoName],
    ['Add-ons', (d.addonLabels || []).join(', ')],
...
module.exports = { mailConfigured, sendDesignEmail, sendOrderEmail, sendAlertEmail, readMailImage, saveLead, readSignupsCsv, notifyLead };

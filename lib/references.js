...
function loadReferences(names) {
  ...
  const all = [...listImages(REF_DIR)];
  try {
    for (const d of fs.readdirSync(REF_DIR, { withFileTypes: true })) {
      if (d.isDirectory() && !d.name.startsWith('_')) all.push(...listImages(path.join(REF_DIR, d.name)));
    }
  } catch (_) {}
  return all.filter((p) => wanted.has(path.basename(p))).map((p) => ({
    buffer: fs.readFileSync(p),
    mimetype: MIME[path.extname(p).toLowerCase()] || 'image/jpeg',
    name: path.basename(p),
    ext: path.extname(p).toLowerCase(),
...
module.exports = { pickReferences, loadReferences, countReferences, REF_DIR };

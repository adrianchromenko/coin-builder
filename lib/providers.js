...
const { buildPrompt, buildDescribedPrompt, guessFinish, finishLabel, normalizeFinish, normalizeCoinShape, normalizeTexts, freeText } = require('./prompt');
const { pickReferences, loadReferences } = require('./references');
const { verifyRender, verifyDescribed } = require('./verify');
...
async function editImage({ apiKey, model, prompt, buffer, mimetype, refs = [], images = null }) {
  // Without any input image (no logo and no reference photos) the plain generation endpoint is the one to use
  const inputs = images || [{ buffer, mimetype, name: 'art-proof.png' }, ...refs.map((r, i) => ({ buffer: r.buffer, mimetype: r.mimetype, name: `factory-coin-${i + 1}${r.ext}` }))];
  if (!inputs.length) return generateImage({ apiKey, model, prompt });
  ...
  // The customer's own image always goes first: it is the image the model stays most faithful to
  for (const img of inputs) form.append('image[]', new Blob([img.buffer], { type: img.mimetype }), img.name);
...
// Text-to-image, for a described coin with no logo and no reference photos on the server
async function generateImage({ apiKey, model, prompt }) {
  const res = await fetch('https://api.openai.com/v1/images/generations', {
    method: 'POST',
    headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ model, prompt, size: process.env.OPENAI_IMAGE_SIZE || '1024x1024', quality: process.env.OPENAI_IMAGE_QUALITY || 'medium', n: 1 }),
    signal: AbortSignal.timeout(240000),
  });
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    const e = new Error(`OpenAI image generation failed (${res.status}, ${model}): ${(data.error && data.error.message) || JSON.stringify(data)}`);
    e.status = res.status;
    throw e;
  }
  const b64 = data && data.data && data.data[0] && data.data[0].b64_json;
...
async function openaiProvider(args) {
  ...
  if (args.mode === 'described') return openaiDescribed(apiKey, args);
  let { buffer, mimetype, finish, color, shape, addons, texts, hasLogo, border, background, centerFirstLineWords } = args;
...
// The described coin: the customer's words become the prompt; their logo (if any) is the first input image.
// Same render -> proofread -> retry loop as the structured coin; the proofreader checks the quoted wording.
// frontImage: the clean render of the front (PNG buffer) when this is the back; it goes in as the first image so the
// back comes out as the same coin turned over. refNames: the factory photos the front was shown, reused here.
// shape: 'round' or 'odd'; an odd-shaped coin is shown only odd-shaped factory photos (see lib/references.js).
async function openaiDescribed(apiKey, { logo, style, description, note = '', shape = 'round', sideName, frontImage = null, refNames = null, onProgress = () => {} }) {
  shape = normalizeCoinShape(shape);
  const refCount = Math.min(Number(process.env.REFERENCE_COUNT ?? 2) || 0, 3);
  const hasLogo = !!(logo && logo.buffer);
  const hasFront = !!frontImage;
  // The back of an odd-shaped coin gets no factory photos: each of those has a different outline, and they competed
  // with the front's. The front is the only coin the model sees, so the front's outline is the only one it can copy.
  const shared = refNames ? loadReferences(refNames) : [];
  const refs = hasFront && shape === 'odd' ? [] : shared.length ? shared : pickReferences(guessFinish(style), refCount, { odd: shape === 'odd' });
  const guide = hasLogo ? `data:${logo.mimetype};base64,${logo.buffer.toString('base64')}` : null;
  // The front, for the outline check on the back of an odd-shaped coin
  const frontGuide = hasFront && shape === 'odd' ? `data:image/png;base64,${frontImage.toString('base64')}` : null;
  const images = [
    ...(hasFront ? [{ buffer: frontImage, mimetype: 'image/png', name: 'front-of-this-coin.png' }] : []),
    ...(hasLogo ? [{ buffer: logo.buffer, mimetype: logo.mimetype, name: 'customer-logo.png' }] : []),
    ...refs.map((r, i) => ({ buffer: r.buffer, mimetype: r.mimetype, name: `factory-coin-${i + 1}${r.ext}` })),
  ];
  const maxAttempts = Math.max(1, Math.min(3, Number(process.env.AI_MAX_ATTEMPTS || 2)));
  const proofread = process.env.AI_VERIFY !== '0';
  ...
  let phrases = [];
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    const built = buildDescribedPrompt({ style, description, note, shape, sideName, hasLogo, hasFront, referenceCount: refs.length, feedback });
    phrases = built.phrases;
    onProgress({ stage: attempt > 1 ? 'retrying' : 'rendering', attempt });
    const { b64, model } = await renderOnce({ apiKey, prompt: built.prompt, images });
    attempts = attempt;
    if (proofread) onProgress({ stage: 'checking', attempt });
    const check = proofread
      ? await verifyDescribed({ apiKey, logo: guide, render: `data:image/png;base64,${b64}`, phrases, hasLogo, front: frontGuide })
      : { checked: false, ok: false, score: 0, lines: [], extraText: [], logoMatch: null, logoIssues: '', borderOk: null, feedback: '' };
    if (check.checked) console.log(`[coin-builder] ${sideName} attempt ${attempt}: ${check.ok ? 'passed proofreading' : 'failed proofreading: ' + check.feedback}`);
    if (!best || check.score > best.check.score) best = { b64, model, check };
    if (!check.checked || check.ok) break;
    feedback = check.feedback;
  }
  const { feedback: _internal, ...check } = best.check;
  return { mimetype: 'image/png', base64: best.b64, check, attempts, model: best.model, phrases, refs: refs.map((r) => r.name) };
}
 ...
 */
async function demoProvider(args) {
  const check = { checked: false, ok: false, lines: [], extraText: [], logoMatch: null, logoIssues: '' };
  if (args.mode === 'described') {
    // Walk through the same stages as a real render, slowly enough that the progress bar can be seen
    const step = async (event) => { (args.onProgress || (() => {}))(event); await new Promise((r) => setTimeout(r, 1200)); };
    await step({ stage: 'rendering', attempt: 1 });
    await step({ stage: 'checking', attempt: 1 });
    const esc = (t) => String(t).replace(/[&<>"']/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&apos;' }[c]));
    const words = freeText(args.description, 90);
    const logo = args.logo && args.logo.buffer ? `<image href="data:${args.logo.mimetype};base64,${args.logo.buffer.toString('base64')}" x="262" y="262" width="500" height="380" preserveAspectRatio="xMidYMid meet"/>` : '';
    // An odd-shaped coin is drawn as an octagon so the option can be seen working without an API key
    const odd = normalizeCoinShape(args.shape) === 'odd';
    const ring = (r, fill, extra = '') => odd
      ? `<polygon points="${Array.from({ length: 8 }, (_, i) => { const a = Math.PI / 8 + (i * Math.PI) / 4; return `${(512 + r * Math.cos(a)).toFixed(1)},${(512 + r * Math.sin(a)).toFixed(1)}`; }).join(' ')}" fill="${fill}"${extra}/>`
      : `<circle cx="512" cy="512" r="${r}" fill="${fill}"${extra}/>`;
  ...
  ${ring(440, '#c9971c')}${ring(400, '#7a5510', ' opacity=".5"')}${ring(380, '#b8871a')}${logo}
  <text x="512" y="200" text-anchor="middle" font-family="Georgia, serif" font-size="34" letter-spacing="4" fill="#fff">${esc(args.sideName === 'back' ? 'BACK' : 'FRONT')} - CUSTOM COIN</text>
  <text x="512" y="720" text-anchor="middle" font-family="Georgia, serif" font-size="22" fill="#fff" opacity=".9">${esc(words)}</text>
  <text x="512" y="960" text-anchor="middle" font-family="Georgia, serif" font-size="30" letter-spacing="6" fill="#f8e27a" opacity=".85">DEMO PREVIEW</text></svg>`;
    return { mimetype: 'image/svg+xml', base64: Buffer.from(svg).toString('base64'), check, attempts: 1, model: 'demo', phrases: [] };
  }
  const { buffer, mimetype, finish } = args;
  const palettes = {

...
async function openaiDescribed(apiKey, { logo, style, description, note = '', shape = 'round', sideName, frontImage = null, refNames = null, onProgress = () => {} }) {
  ...
  const refs = shared.length ? shared : pickReferences(guessFinish(style), refCount, { odd: shape === 'odd' });
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    const check = proofread
      ? await verifyDescribed({ apiKey, logo: guide, render: `data:image/png;base64,${b64}`, phrases, hasLogo })
      : { checked: false, ok: false, score: 0, lines: [], extraText: [], logoMatch: null, logoIssues: '', borderOk: null, feedback: '' };

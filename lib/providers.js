...
async function openaiDescribed(apiKey, { logo, purpose, style, description, sideName, onProgress = () => {} }) {
  const refCount = Math.min(Number(process.env.REFERENCE_COUNT ?? 2) || 0, 3);
  const refs = pickReferences(guessFinish(style), refCount);
  const hasLogo = !!(logo && logo.buffer);
  const guide = hasLogo ? `data:${logo.mimetype};base64,${logo.buffer.toString('base64')}` : null;
  const images = [...(hasLogo ? [{ buffer: logo.buffer, mimetype: logo.mimetype, name: 'customer-logo.png' }] : []), ...refs.map((r, i) => ({ buffer: r.buffer, mimetype: r.mimetype, name: `factory-coin-${i + 1}${r.ext}` }))];
  const maxAttempts = Math.max(1, Math.min(3, Number(process.env.AI_MAX_ATTEMPTS || 2)));
  const proofread = process.env.AI_VERIFY !== '0';
  ...
  let phrases = [];
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    const built = buildDescribedPrompt({ purpose, style, description, sideName, hasLogo, referenceCount: refs.length, feedback });
  ...
  const { feedback: _internal, ...check } = best.check;
  return { mimetype: 'image/png', base64: best.b64, check, attempts, model: best.model, phrases };
}

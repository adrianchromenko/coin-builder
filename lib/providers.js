...
const { buildPrompt, buildDescribedPrompt, guessFinish, finishLabel, normalizeFinish, normalizeTexts, freeText, PURPOSES } = require('./prompt');
...
async function openaiDescribed(apiKey, { logo, purpose, style, description, sideName, frontImage = null, refNames = null, onProgress = () => {} }) {
  const refCount = Math.min(Number(process.env.REFERENCE_COUNT ?? 2) || 0, 3);
  const shared = refNames ? loadReferences(refNames) : [];
  const refs = shared.length ? shared : pickReferences(guessFinish(style), refCount);
  ...
  let phrases = [];
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    const built = buildDescribedPrompt({ purpose, style, description, sideName, hasLogo, hasFront, referenceCount: refs.length, feedback });
...
async function demoProvider(args) {
  ...
  if (args.mode === 'described') {
    ...
    const purpose = args.purpose && PURPOSES[args.purpose] ? PURPOSES[args.purpose].label.toUpperCase() : 'CUSTOM COIN';
  ...
  <circle cx="512" cy="512" r="440" fill="#c9971c"/><circle cx="512" cy="512" r="400" fill="#7a5510" opacity=".5"/><circle cx="512" cy="512" r="380" fill="#b8871a"/>${logo}
  <text x="512" y="200" text-anchor="middle" font-family="Georgia, serif" font-size="34" letter-spacing="4" fill="#fff">${esc(args.sideName === 'back' ? 'BACK' : 'FRONT')} - ${esc(purpose)}</text>

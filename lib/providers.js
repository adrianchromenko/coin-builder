...
async function openaiProvider({ buffer, mimetype, finish, color, shape, addons, texts, hasLogo, border, background, centerFirstLineWords }) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    const check = proofread
      ? await verifyRender({ apiKey, guide, render: `data:image/png;base64,${b64}`, texts, hasLogo })
      : { checked: false, ok: false, score: 0, lines: [], extraText: [], logoMatch: null, logoIssues: '', feedback: '' };

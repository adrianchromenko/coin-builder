...
async function openaiDescribed(apiKey, { logo, style, description, shape = 'round', sideName, frontImage = null, refNames = null, onProgress = () => {} }) {
  ...
  let phrases = [];
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    const built = buildDescribedPrompt({ style, description, shape, sideName, hasLogo, hasFront, referenceCount: refs.length, feedback });
